...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #f7f9fb; padding: 2rem;">
  <div style="max-width: 480px; margin: 0 auto; background: white; border-radius: 12px; border: 1px solid #e2e8f0; overflow: hidden;">
    <div style="background: #1e1e1e; padding: 1.5rem 2rem;">
      <h1 style="color: white; margin: 0; font-size: 1.25rem;">Your re:AI verification code</h1>
    </div>
    <div style="padding: 2rem;">
      <p style="color: #475569;">Use this code to sign in to your re:AI account:</p>
      <div style="background: #f1f5f9; border-radius: 8px; padding: 1.5rem; text-align: center; margin: 1.5rem 0;">
        <span style="font-size: 2rem; font-weight: 800; letter-spacing: 0.25em; color: #1e1e1e; font-family: monospace;">${code}</span>
      </div>
...
export default async (req: Request) => {
  ...
  if (resendApiKey) {
    const fromEmail = runtimeEnv.RESEND_FROM_EMAIL || "re:AI <reports@re-ai.app>";
    const result = await sendEmail(resendApiKey, {
      from: fromEmail,
      to: email,
      subject: "Your re:AI verification code",
